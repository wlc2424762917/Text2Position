...
from models.mask3d_modules.common import conv
from models.position_embedding import PositionEmbeddingCoordsSine
from third_party.pointnet2.pointnet2_utils import furthest_point_sample
from models.mask3d_modules.helpers_3detr import GenericMLP
from torch_scatter import scatter_mean, scatter_max, scatter_min
from torch.cuda.amp import autocast
import hydra

from models.res16unet import Res16UNet18B
...
class Mask3D(nn.Module):
    def __init__(
        ...
        config = {
            ...
            'out_channels': 22,
            'out_fpn': True,
            'D': 3,  # Assuming 3D data. Adjust this if working with 4D (spatiotemporal) data.
        }

        self.backbone = Res16UNet18B(
    ...
    def forward(
        ...
        if self.non_parametric_queries:
            ...
            if not self.use_np_features:
                ...
                )  # get the features at the sampled points, Batch, Dim, queries
...
def load_checkpoint_with_missing_or_exsessive_keys(checkpoint, model):
    ...
    for key in state_dict_ori.keys():
        if "model." in key:
            state_dict[key.replace("model.", "")] = state_dict_ori[key]
...
if __name__ == "__main__":
    ...
    def create_mock_input(batch_size, num_points, num_features, voxel_size=0.05):
        """
        创建模拟输入。
        """
        coords = []
        ...
        for _ in range(batch_size):
            coord = np.random.rand(num_points, 3) * 100  # 随机生成点坐标
            feature = np.random.rand(num_points, num_features)  # 随机生成特征
            coords.append(coord)
            features.append(feature)
        coords = np.vstack(coords)
        coords = np.floor(coords / voxel_size)  # 量化坐标
        coords = np.concatenate((np.zeros((coords.shape[0], 1)), coords), axis=1)  # 添加批次索引
    ...
    def test_mask3d():
        ...
        # 创建模拟输入
        coords, features = create_mock_input(batch_size, num_points, num_features, voxel_size)
        coords = torch.from_numpy(coords).int()
        features = torch.from_numpy(features).float()

        # 创建稀疏张量
        stensor = SparseTensor(features=features, coordinates=coords, device="cuda")
        ...
        model = Mask3D(
            ...
            non_parametric_queries=non_parametric_queries,
            train_on_segments=train_on_segments,
            normalize_pos_enc=normalize_pos_enc,
            use_level_embed=use_level_embed,
            scatter_type=scatter_type,
            hlevels=hlevels,
            use_np_features=use_np_features,
            voxel_size=voxel_size,
            max_sample_size=max_sample_size,
            random_queries=random_queries,
            gauss_scale=gauss_scale,
            random_query_both=random_query_both,
            random_normal=random_normal,
        )
        model.to("cuda")
        print(model)
        model = load_checkpoint_with_missing_or_exsessive_keys("/home/wanglichao/Mask3D/saved/t2p_015/last-epoch.ckpt", model)

        # 前向传递
        # output = model(stensor, raw_coordinates=features.to("cuda"))
        #
        # print("Output keys:", list(output.keys()))
        # print(f"queries: {output['queries'].shape}")
        # print(f"pred_logits: {output['pred_logits'].shape}")
        # print(f"pred_masks: {len(output['pred_masks']), output['pred_masks'][0].shape}")
        # print(f"sampled_coords: {len(output['sampled_coords'])}")
